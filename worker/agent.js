...
export async function runAgent({ decide, search, details, emit = () => {} }, message, history = []) {
  const observations = [];
  const places = new Map();
  const seen = new Set();
  for (let step = 0; step <= 8; step++) {
    emit({type:"status", message: step ? "Reviewing results…" : "Understanding your request…"});
    const action = JSON.parse(await decide(AGENT_PROMPT, JSON.stringify({
      message, history: history.filter(x => ["user", "assistant"].includes(x.role)).slice(-12).map(x => ({role:x.role, content:String(x.content).slice(-8000), places:(x.places || []).map(p => ({place_id:p.place_id,name:p.name,address:p.address})), suggestions:x.suggestions || []})),
    ...
    try {
      if (action.action === "search" && typeof action.query === "string" && action.query.trim() && action.query.length <= 300) {
        emit({type:"tool", id:step, tool:"search", state:"running", message:`Searching Google Maps for ${action.query}`});
        const found = await search(action.query, {open_now: action.open_now === true});
        emit({type:"tool", id:step, tool:"search", state:"complete", message:`Found ${found.length} places for ${action.query}`});
        ...
        emit({type:"tool", id:step, tool:"details", state:"running", message:`Checking details for ${places.get(action.place_id).name}`});
        const data = await details(action.place_id);
        emit({type:"tool", id:step, tool:"details", state:"complete", message:`Checked details for ${places.get(action.place_id).name}`});
      ...
      emit({type:"tool", id:step, tool:action.action, state:"error", message:"Place lookup failed. Trying to continue with available information."});
      observations.push({action, error:"Place lookup failed. Do not invent missing data."});
    }
  }
