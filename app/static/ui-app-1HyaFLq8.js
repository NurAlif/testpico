...
async function ye() {
	U?.disconnect(), p.querySelectorAll(".msg").forEach((e) => e.remove()), m.hidden = !1, C = [], T = null, A(""), a.value = "", a.style.height = "auto", j(), f.scrollTo({ top: 0 }), a.focus(), M(!0);
	try {
		await ve(), await Q();
	} catch (e) {
		A(`Could not start a saved conversation: ${e.message}`);
	} finally {
		M(!1);
	}
}
