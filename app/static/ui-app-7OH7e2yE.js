...
function X() {
	...
	e.querySelector(".auth-submit-label").textContent = Y ? "Create account" : "Log in", t.querySelector(".auth-toggle-label").textContent = Y ? "Log in instead" : "Create an account", document.querySelector("#auth-toggle-description").textContent = Y ? "Already have an account?" : "New to Wander Pico?", e.querySelector(".auth-icon-login").toggleAttribute("hidden", Y), e.querySelector(".auth-icon-register").toggleAttribute("hidden", !Y), t.querySelector(".auth-icon-login").toggleAttribute("hidden", !Y), t.querySelector(".auth-icon-register").toggleAttribute("hidden", Y), t.href = Y ? "#login" : "#register", K.textContent = "", document.querySelector("#auth-success").textContent = "";
}
...
G.addEventListener("submit", async (e) => {
	...
	t.disabled = !0, K.textContent = "", document.querySelector("#auth-success").textContent = "";
	try {
		...
		Y ? (await O("/api/auth/register", {
			...
			document.querySelector("#auth-success").textContent = "Your account is ready! Log in to start exploring.";
